...
function buildDepthAndSectionsBlock(depth: string, selectedSections: string[] | undefined): string {
  ...
  block += `\n\n⛔ NEVER PROPOSE OR IMPLY THESE FEATURES — we do NOT support them:\n- Blog / CMS / articles publishing (no back-office to add new posts)\n- Online store / e-commerce / products / cart / checkout / Stripe products\n- Booking system / calendar / appointment scheduling (no calendar backend)\n- Events calendar / events publishing\n- Downloadable resources / file library / whitepapers upload\n- Member areas, login-gated content, user accounts on the generated site\nIf the user asks for any of the above, briefly say we currently generate marketing pages only; recommend Shopify for e-commerce, Cal.com for booking, Substack/Beehiiv for blogs. Do NOT add their keys to business.selectedSections — they are not in the library anyway.\nFor a restaurant/café: "menu" is display-only, no ordering. For services: contact form collects inquiries but there is no real-time booking.`;
