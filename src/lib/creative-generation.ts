...
- DISPLAY-ONLY, NO FAKE FUNCTIONALITY: this is a marketing site with NO backend store, CMS, calendar, payment, or auth. Never render controls that imply a working backend that does not exist. Specifically forbidden: "Add to cart" / "Buy now" / checkout / quantity steppers / cart icons; a blog index with "Read more" links to article pages that do not exist; event "Register" / ticket-purchase buttons; "Download" buttons for files we do not host; login / sign-up / account / member-area UI. If the brand sells products, render a SHOWCASE (image, name, price as plain text, short description) whose call to action is "Inquire" or "Order via email/WhatsApp", never a cart. A restaurant menu is display-only (no online ordering).
- NO CONTACT FORMS OR INPUT FIELDS: there is NO form backend. Never render a contact form, lead/inquiry form, or newsletter signup that collects name, surname, email, phone, message, or question inputs. Do not output <form>, <input>, or <textarea> elements at all. For getting in touch, the call to action is a prominent mailto: link, tel: link, or WhatsApp link (https://wa.me/NUMBER), never a form. A "Contact" section is great as long as it shows email/phone/social as real clickable links, not fields to fill in.
- Every visible control MUST do something real: scroll to a section, open mailto:/tel:/a WhatsApp link, or trigger the booking element (data-book). The only interactive backend that exists is the booking widget (data-book), handled for you.`;
...
CRITICAL — DO NOT INVENT TRACKS: Render ONLY the embeds in the URL list above — one iframe per URL, nothing more. Do NOT write track listings, fake release titles, fake album names, fake catalog numbers, or any text that looks like a discography unless those titles are explicitly in the brief content (e.g. brief.content.releases or [EXACT] sacred fields). Plausible-sounding fake titles ("Club Original", "Underground Protocol", "Volume One", "EP-23") are FORBIDDEN — readers will recognize them as fake and it destroys credibility. If you need more visual density around the embeds, use real assets from the brief (artist quotes, press mentions, tour dates, the artist's tagline) — never invented track metadata.`;
  ...
  if (features.ecommerce) featureNotes.push('- The brand sells products but there is NO store backend. Render products as a showcase/catalog (image, name, price as plain text, short description). The CTA is "Inquire" or "Order via email/WhatsApp" (a mailto/tel/WhatsApp link, never a form). No add-to-cart, cart icon, checkout, or quantity controls.');
  ...
  if (features.booking || brief?.preferences?.primary_goal === 'book') featureNotes.push('- Booking: render the booking CTA as a plain element with data-book="true" (the popup is wired post-generation). Do NOT embed a third-party booking iframe yourself. Put a mailto/tel/WhatsApp fallback next to it (never a form).');
